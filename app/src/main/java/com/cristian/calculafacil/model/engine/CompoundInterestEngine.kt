...
object CompoundInterestEngine : CalculatorEngine {
    override val id = "compound_interest"
    override val titleRes = R.string.calc_compound_title
    override val descriptionRes = R.string.calc_compound_desc
    override val icon = Icons.Filled.CurrencyExchange
    override val category = Category.Finance

    override val fields = listOf(
        CalcField("principal", R.string.calc_compound_field_principal, FieldType.Money),
        CalcField("rate", R.string.calc_compound_field_rate, FieldType.Percent),
        CalcField("months", R.string.calc_compound_field_months, FieldType.Integer),
    )
    ...
    override fun calculate(values: Map<String, Double>): CalcOutcome {
        ...
        val amount = Interest.compoundAmount(principal, rate, months)
        val interest = Interest.compoundInterest(principal, rate, months)
        if (!amount.isFinite()) return CalcOutcome.Error(R.string.error_number_too_large)
        return CalcOutcome.Success(
            rows = listOf(
                ResultRow(R.string.calc_compound_amount, Format.money(amount)),
                ResultRow(R.string.calc_compound_interest, Format.money(interest)),
            ),
        )
    }
}